...
def sales_persons(sales_person: pd.DataFrame, company: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    ...
    orders = company.merge(orders,how='inner', on='com_id')
    sales_person = orders.merge(sales_person, how='outer', on='sales_id')
    sales_person = sales_person.fillna({'amount': 0})
    sales_person = sales_person[sales_person['amount'] == 0]
    return sales_person[['name']]

print(sales_persons(sales_person, company, orders))